...
<head>
  ...
  <title>{{config('app.name')}} - Painel do cliente {{isset($meta_title) ? (($meta_title<>'') ? ' - ' . $meta_title : '') : ''  }}</title>
  <meta name="description" content="{{isset($meta_description) ? (($meta_description<>'') ? $meta_description : '') : 'i12 Sistema - Painel do cliente'  }}">
  ...
  <meta name="csrf-token" content="{{ csrf_token() }}">
    ...
    if(Auth::guard('painelcliente')->check())
        if(Auth::guard('painelcliente')->getSession()->has('cliente'))
            $idclienteatual = Auth::guard('painelcliente')->getSession()->get('cliente')->codcliente;
    @endphp
  
  @if(Auth::guard('painelcliente')->check())
  <meta name="contato" content="{{ json_encode(['id' => Auth::guard('painelcliente')->User()->id, 
                                               'nome' => Auth::guard('painelcliente')->User()->nome, 
                                               'cpfcnpj' => Auth::guard('painelcliente')->User()->cpfcnpj,
                                               'permissaopainel' => Auth::guard('painelcliente')->User()->permissoes()->where('idcliente', $idclienteatual)->get()->pluck('idpermissao')->toArray()
                                               ]) }}">
  @endif
  @if(Auth::guard('painelcliente')->check())
    @if(Auth::guard('painelcliente')->getSession()->has('cliente'))
        <meta name="cliente" content="{{  Auth::guard('painelcliente')->getSession()->get('cliente') }}">
    @endif
  @endif
...
<div id="app">
    ...
    @endforeach
    <app-painelcliente></app-painelcliente>
</div>
<!-- ./wrapper -->
...
<script>
    ...
    var url = "{!! url('/') !!}";
    var origem = "{!! Auth::guard('painelcliente')->getSession()->has('origem') ? 'icom' : '' !!}"
</script>
