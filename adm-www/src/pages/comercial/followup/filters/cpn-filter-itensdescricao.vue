      ...
      <q-card-section>
        <div class="fit row wrap full-width">
          <div class="col-10 q-mb-sm q-mr-sm ">
            <selectitensdescricao outlined label="Descrição dos itens" nullabled :dense="!$q.platform.is.mobile" v-model="model"  />
          </div>
          <div class="col-1">
        ...
        <div class="fit row wrap q-col-gutter-x-md q-col-gutter-y-md q-pb-md">
          <div class="col-12 q-mb-sm">
            <div v-for="(item, key) in lista" :key="key" class="full-width">
              <q-chip removable @remove="actRemove(key)" class="full-width">{{item.itemdescricao}}
              </q-chip>
            </div>
          </div>
        </div>
...
import selectitensdescricao from './cnp-select-itensdescricao'
export default {
  name: 'dialog.followup.filter.itensdescricao',
  components: {
    selectitensdescricao
  },
  props: {
    config: {
      type: Object,
  ...
  async mounted () {
    ...
    if (app.config.value ? app.config.value.length > 0 : false) {
      var ret = await app.dataset.fetchlistitensdescricao('', app.config.value)
      if (ret.ok) {
  ...
  methods: {
    ...
    async actAdd () {
      ...
      if (app.model) {
        ...
        for (let index = 0; index < app.lista.length; index++) {
          ...
          if ((element.itemdescricao.toString().toLowerCase()) === app.model.itemdescricao.toString().toLowerCase()) {
            idx = index
            break
          }
        }
    ...
    async onOKClick () {
      try {
        ...
        dados = []
        for (let index = 0; index < app.lista.length; index++) {
          dados.push(app.lista[index].itemdescricao)
        }
