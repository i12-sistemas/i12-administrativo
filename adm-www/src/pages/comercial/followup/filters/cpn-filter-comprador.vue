    ...
    <q-card class="q-dialog-plugin" bordered :style="$q.screen.lt.sm ? '' : 'min-width: 50vw'">
      ...
      <q-card-section class="scroll" style="max-height: 50vh" >
        <div class="fit row wrap full-width">
          <div class="col-12">
            <q-table :data="rows" :columns="[{ name: 'comprador', align: 'left', label: 'Compradores', field: 'comprador' }]" dense bordered flat :loading="loading"
              id="sticky-table" row-key="comprador" :rows-per-page-options="[0]"
...
export default {
  name: 'dialog.followup.filter.comprador',
  components: {
  },
  props: {
    config: {
  ...
  async mounted () {
    ...
    if (app.config.value ? app.config.value.length > 0 : false) {
      ...
      for (let index = 0; index < app.config.value.length; index++) {
        app.selected_rows.push({ comprador: app.config.value[index] })
      }
    } else {
      app.value = []
    }
  ...
  methods: {
    async onRefresh () {
      var app = this
      app.loading = true
      var ret = await app.dataset.fetchlistcompradores()
      if (ret.ok) {
        app.rows = []
        for (let index = 0; index < ret.data.length; index++) {
          const element = ret.data[index]
          app.rows.push({ comprador: element })
        }
      }
      app.loading = false
    },
