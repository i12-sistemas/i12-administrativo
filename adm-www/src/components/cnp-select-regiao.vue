        ...
        <template v-slot:option="scope">
          <q-item v-bind="scope.itemProps" v-on="scope.itemEvents" :dense="dense" class="border-bottom-separator" >
            <q-item-section avatar>
              <q-avatar color="grey-5" text-color="white" font-size="14px" >{{scope.opt.id}}</q-avatar>
            </q-item-section>
            <q-item-section>
              <q-item-label >{{scope.opt.regiao}}</q-item-label>
              <q-item-label caption v-if="scope.opt.cidadescount ? scope.opt.cidadescount > 0 : false" >{{ scope.opt.cidadescount }} cidades</q-item-label>
            </q-item-section>
          </q-item>
        </template>
...
<script>
import Regioes from 'src/mvc/collections/regioes.js'
export default {
  components: {
  },
  props: [
    'value', 'dense', 'outlined', 'label', 'clearable', 'nullabled', 'loading', 'disable'
  ],
  data () {
    var dataset = new Regioes()
    return {
      dataset,
      modelselect: null,
      options: null,
