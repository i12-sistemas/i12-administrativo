    ...
    <q-card class="q-dialog-plugin" bordered>
      ...
      <q-card-section>
        <div class="fit row wrap q-col-gutter-x-md q-col-gutter-y-md q-pb-md">
          <div class="col-12">
            <q-input outlined v-model="value" label="Valor" stack-label :dense="!$q.platform.is.mobile" ref="txtvalue" autofocus @keyup.enter="onOKClick"
...
export default {
  ...
  props: {
    dados: {
      type: Object,
      default: function () {
        return { value: 0, prefix: null, mask: '' }
      }
    }
  },
  data () {
  ...
  async mounted () {
    ...
    if (app.dados) {
      ...
      if (app.dados.value) app.value = parseFloat(app.dados.value).toFixed(app.decimal)
  ...
  methods: {
    ...
    onOKClick () {
      try {
        var app = this
        var vi = parseFloat(app.$helpers.strToCurr(app.value)).toFixed(app.decimal)
      } catch (error) {
