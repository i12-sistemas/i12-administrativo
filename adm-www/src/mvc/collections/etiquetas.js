...
class Etiquetas {
  ...
  async fetch (pContinuos = false) {
    ...
    var self = this
    if (!pContinuos) self.limpardados()
    if (self.orderby ? typeof self.orderby !== 'undefined' : false) self.query.orderby = JSON.stringify(self.orderby)

    let ret = await Vue.prototype.$axios.get('v1/etiquetas', { params: self.query }).then(response => {
