...
class Atendimentos {
  ...
  async fetch (pContinuos = false) {
    ...
    if (self.situacao) {
      if (self.situacao !== null) params['situacao'] = self.situacao
    }
    if (self.filter) {
    ...
    let ret = await Vue.prototype.$axios.get('v1/painelcliente/atendimentos', { params: params }).then(response => {
