  ...
  <q-dialog ref="dialog" @hide="onDialogHide" :maximized="$q.platform.is.mobile">
    <q-card class="q-dialog-plugin" :class="$q.platform.is.mobile ? '' : 'full-70'">
      <q-bar class="bg-primary text-white" v-if="!$q.platform.is.mobile">
        <span class="ellipsis text-subtitle2 q-ml-xs">Cadastro :: {{ (!loading ? (adding ? 'Novo cadastro' : (dataset ? dataset.descricao_old +  ' [id: ' + dataset.id + ']' : '?')) : 'Consultando')}}</span>
        <q-space />
      ...
      <q-toolbar class="bg-primary text-white" v-if="$q.platform.is.mobile">
        <q-toolbar-title>
          Cadastro :: {{ (!loading ? (adding ? 'Novo' : (dataset ? dataset.descricao_old : '?')) : 'Consultando')}}
        </q-toolbar-title>
...
export default {
  name: 'followup.cadastro.erros.edit.dialog',
  components: {
    formedit
  },
  props: {
