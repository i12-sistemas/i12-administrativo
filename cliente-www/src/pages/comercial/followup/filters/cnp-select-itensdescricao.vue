  ...
  <q-select :dense="dense" :outlined="outlined" v-model="modelselect" input-debounce="300"
      ...
      >
        <template v-slot:selected-item="scope">
            <div  v-if="scope.opt ? scope.opt.itemdescricao !== '' : false" class="ellipsis text-uppercase no-wrap"  >
              {{scope.opt.itemdescricao }}
              <q-tooltip>
                <div>{{scope.opt.itemdescricao}}</div>
              </q-tooltip>
            </div>
        </template>
        <template v-slot:option="scope">
          <q-item v-bind="scope.itemProps" v-on="scope.itemEvents" dense class="border-bottom-separator full-width" >
            <q-item-section>
              <q-item-label>{{scope.opt.itemdescricao}}</q-item-label>
            </q-item-section>
          </q-item>
        </template>
...
export default {
  name: 'cnp.followup.filter.itensdescricao',
  components: {
  },
  props: [
  ...
  computed: {
    selecao: function () {
      ...
      for (let index = 0; index < app.options.length; index++) {
        ...
        if (app.registroselecionado === element.itemdescricao) {
          sel = element.dados
          break
        }
      }
  ...
  methods: {
    ...
    async refreshData (val, update) {
      ...
      var ret = await app.dataset.fetchlistitensdescricao(val)
      if (ret.ok) {
        update(() => {
          this.options = []
